...
def download_misc_text_files(sftp_client, local_directory,download_date):
    text_file_path = 'locations.txt'  # Path to your text file containing folder names

    with open(text_file_path, 'r') as f:
        folders = f.readlines()

    for folder in folders:
        print(f"[INFO]-->> Currently, downloading from this  ⬇️  { folder } ⬇️")
        folder = folder.strip()
        sftp_path = '/' + folder
        sftp_client.chdir(sftp_path) 
        files = sftp_client.listdir_attr()
        misc_text_files = [file for file in files if file.filename.endswith('.txt') and 'EPR_Prod_Misc' in file.filename]
        if misc_text_files:
